...
#   alt + {a,f}
#       /home/sunn/scripts/move.sh {-p,-n} -x -s 20
#   
#   alt + {s,d}
#       /home/sunn/scripts/move.sh {-p,-n} -y -s 20
...
WINDOW_STATUS="$(bspc query -T -w | awk '/^.* [a-zA-Z\-]{8} \*$/{print 
$8}')" 
...
# The window is tiled modified split ratio
	...
	$POSITIVE && sign="+" || sign="-"
	bspc window -e ${switch[0]} ${sign}${SIZE} ||  bspc window -e 
${switch[1]} ${sign}${SIZE}
fi
